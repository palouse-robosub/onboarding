...
class AtmosphericPressurePublisher : public rclcpp::Node {
  public:
    ...
    // makes the HTTP get Request and returns a plaintext string of the METAR
    ...
    double GetAtmosphericPressure() {
        ...
        if (!hour || hour != currentHour) {    // METAR is updated once per hour
                                               // so we check if we've already
                                               // gotten it this hour
